...
function copyPt(number) {
    let copyTextFrom = document.getElementById('copyFrom');
    let copyNotif = document.querySelectorAll('.notif')
    let optionNumber;
    copyTextFrom.value = option.power_tunnel;
...
let coderList = "";
for(let i = 0; i < option.coder.length; i++) {
    coderList += option.coder[i].concat(', ')
}
...
coderServer.innerHTML = coderList.slice(0, coderList.length - 2);
...
discordLink.href = option.discord;
whatsappLink.href = option.whatsapp;